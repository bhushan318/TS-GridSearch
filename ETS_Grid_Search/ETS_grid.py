...
class ETSGRID:
    ...
    ''' ETS grid is a calss to find best tresd and seasonality type for given time series'''
    ...
    def ETS_singlerun(self, trend_type, seasonal_type):
        ''' for given trend and seasonal type model will be fitted and its results are returned
        if particular combination of parameters are not sutiable then function will return nan values'''
        try:
            model= ExponentialSmoothing( endog=self.train, trend=trend_type, damped=True, seasonal=seasonal_type, 
                                    seasonal_periods=self.seasonal_period, dates=None, freq=None, missing='none').fit()
            aic = model.aic
            bic = model.bic
    ...
    def ETSGridSearch (self):
        ...
        result_table_ETS=pd.DataFrame(np.vectorize(self.ETS_singlerun)(Trend_seasonal['trend'],
                                                            Trend_seasonal['seasonal'])).transpose()
        result_table_ETS.columns=['Trend','Seasonal','seasonal_period',"AIC","BIC",
                                  'error_mse_fitting',
                                  'error_mae_fitting',
                                  'error_mape_fitting',
                                  'error_mse_prediction',
        ...
        result_table_ETS = result_table_ETS.sort_values(by='AIC', ascending=True).reset_index(drop=True)
        result_table_ETS_1=result_table_ETS[~result_table_ETS.Trend.str.contains("Fals")]
